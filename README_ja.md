...
> 24 量子ビットの `efficient_su2` 回路（statevector・256 shots・ウォームアップ後 5 回計測の中央値、
> 本マシン実測）では **GPU 約 0.04 秒 vs CPU 約 0.84 秒＝約 21 倍高速**でした
> （初回の GPU 呼び出しのみ cuStateVec/CUDA の初期化で約 0.3 秒。上のベンチマークの小さな回路を
> 多数回投げる用途では、この初期化が毎回効いて GPU が不利になります）。
